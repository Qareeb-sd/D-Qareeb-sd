...
export async function getCurrentPos(): Promise<GeoPos | null> {
  if (isNative) {
    try {
      ...
      // حاجز الدقّة: نقبل قراءات GPS الحقيقية فقط (≤ 300م) ونرفض موقع الشبكة
      // التقريبي (قد يبعد كيلومترات/بلداناً مع شريحة أجنبية) — لأن هذا الموقع يُبثّ
      // للراكب ويُغذّي مسار الالتقاط. إن لم نحصل على قراءة دقيقة خلال المهلة → null
      // (فيحدّد المستخدم يدوياً على الخريطة).
      const ACCEPT_M = 300
