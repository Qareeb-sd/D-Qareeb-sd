...
// مركز شكل السودان بصرياً (المركز الهندسي ≈ 499,401) — الدبوس يتوسّط الخريطة
// وطرفه/رأسه يحيطان بالمركز، ومنه تنطلق خطوط التوصيل لبقيّة المدن.
const KHARTOUM = { x: 500, y: 424 }
const CITIES = [
  { x: 924, y: 183 }, // بورتسودان
  { x: 535, y: 210 }, // دنقلا
  { x: 877, y: 433 }, // كسلا
  { x: 210, y: 637 }, // نيالا
  { x: 519, y: 569 }, // الأبيّض
...
export default function SudanMap({ className }: { className?: string }) {
  const { x, y } = KHARTOUM
  const head = y - 34 // مركز رأس الدبوس فوق الطرف
