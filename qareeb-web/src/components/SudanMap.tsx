/**
 * خريطة السودان الحقيقية (شكل الحدود الفعلي) وعليها خطوط التوصيل الذهبية
 * المتحرّكة من الخرطوم إلى بقية المدن، ودبوس "قريب" النابض على الخرطوم.
 *
 * مسار الحدود مصدره مكتبة mapsicon مفتوحة المصدر (مبسّط بـ potrace).
 * الإحداثيات في فضاء 1024×1024؛ نقاط المدن مُسقطة تقريبياً من خطوط الطول والعرض.
 */

// مسار حدود السودان — يُرسم داخل مجموعة محوّلة (potrace: قلب رأسي + تصغير).
const SUDAN_D =
...
// مدن على خطوط التوصيل (إحداثيات فضاء 1024) — الخرطوم هي المحور.
const KHARTOUM = { x: 627, y: 500 }
const CITIES = [
  { x: 830, y: 225 }, // بورتسودان (البحر الأحمر)
  { x: 527, y: 235 }, // دنقلا (الشمال)
  { x: 800, y: 515 }, // كسلا (الشرق)
  { x: 255, y: 780 }, // نيالا (الغرب)
  { x: 515, y: 690 }, // الأبيض (الوسط/الجنوب)
]

export default function SudanMap({ className }: { className?: string }) {
  return (
    <svg
      viewBox="70 0 892 1024"
      className={className}
      fill="none"
    ...
    >
      ...
      <g transform="translate(0,1024) scale(0.1,-0.1)">
        <path d={SUDAN_D} fill="#1B6B3F" fillOpacity="0.16" stroke="#1B6B3F" strokeWidth="18" strokeOpacity="0.55" />
      </g>

      {/* خطوط التوصيل الذهبية من الخرطوم */}
      {CITIES.map((c, i) => (
        <line
          key={i}
          x1={KHARTOUM.x}
          ...
          opacity="0.7"
        >
          <animate attributeName="stroke-dashoffset" values="56;0" dur="1.6s" repeatCount="indefinite" />
        </line>
      ))}

      {/* نقاط المدن */}
      {CITIES.map((c, i) => (
        <circle key={`d${i}`} cx={c.x} cy={c.y} r="11" fill="#1B6B3F" opacity="0.6" />
      ))}

      {/* دبوس قريب النابض على الخرطوم */}
      <ellipse cx={KHARTOUM.x} cy={KHARTOUM.y} rx="34" ry="20" fill="#1B6B3F" opacity="0.15">
        <animate attributeName="rx" values="34;48;34" dur="2s" repeatCount="indefinite" />
        <animate attributeName="ry" values="20;28;20" dur="2s" repeatCount="indefinite" />
        <animate attributeName="opacity" values="0.15;0.04;0.15" dur="2s" repeatCount="indefinite" />
      </ellipse>
      <path
        d={`M${KHARTOUM.x} ${KHARTOUM.y - 52}c-16 0-29 12.4-29 27.7 0 19.6 25 43 27.5 45.3a2 2 0 0 0 3 0c2.5-2.3 27.5-25.7 27.5-45.3 0-15.3-13-27.7-29-27.7Z`}
        fill="#1B6B3F"
      />
      <circle cx={KHARTOUM.x} cy={KHARTOUM.y - 25} r="13" fill="none" stroke="#C9A138" strokeWidth="5" />
      <circle cx={KHARTOUM.x} cy={KHARTOUM.y - 25} r="6" fill="#C9A138" />
    </svg>
  )
}
